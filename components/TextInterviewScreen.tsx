...
export default function TextInterviewScreen({ role, initialQuestions, onComplete }: TextInterviewScreenProps) {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    // 1. Thêm câu trả lời của user vào chat logs
    setChatLogs((prev) => [...prev, { sender: "user", text: answer }]);

    const currentQuestion = questions[currentIdx];

    try {
      // 2. Gọi API chấm điểm câu trả lời hiện tại
      ...
      if (!res.ok) throw new Error("Không thể đánh giá câu trả lời.");
      const evaluation = await res.json(); // { score, feedback, suggestedAnswer, followUpQuestion }

      // 3. Cập nhật kết quả vào danh sách câu hỏi để lưu trạng thái
      ...
      // 4. Thêm đánh giá vào chat log ngay dưới câu trả lời của user
      setChatLogs((prev) => {
        const copy = [...prev];
        // Cập nhật log cuối (của user) để đính kèm đánh giá
        copy[copy.length - 1] = {
          ...copy[copy.length - 1],
          evaluation: {
            score: evaluation.score,
      ...
      // 5. Kiểm tra xem có câu hỏi phụ (follow-up) hay không
      if (evaluation.followUpQuestion) {
        // Tạo câu hỏi phụ xen kẽ vào danh sách câu hỏi
        ...
        // Chèn câu hỏi phụ ngay sau vị trí hiện tại
        ...
        // Thêm câu hỏi phụ vào chat log
        setChatLogs((prev) => [
          ...prev,
          {
            sender: "ai",
            text: `[Hỏi phụ] Để làm rõ hơn ý của bạn:\n\n"${evaluation.followUpQuestion}"`,
          },
        ]);
      } else {
        // Nếu không có câu hỏi phụ, chuyển sang câu tiếp theo
        ...
        if (nextIdx < updatedQuestions.length) {
          ...
          setChatLogs((prev) => [
            ...
            {
              sender: "ai",
              text: `Cảm ơn bạn. Hãy tiếp tục với câu hỏi tiếp theo:\n\n"${updatedQuestions[nextIdx].text}"`,
            },
          ]);
        } else {
          // Kết thúc toàn bộ câu hỏi
          setChatLogs((prev) => [
            ...prev,
            {
              sender: "ai",
              text: `Chúng ta đã hoàn thành tất cả các câu hỏi phỏng vấn cho vị trí ${role}. Cảm ơn câu trả lời của bạn! Bạn có thể nhấn nút "Xem kết quả phỏng vấn" phía dưới để xem bảng điểm chi tiết.`,
            },
          ]);
        }
      }
      ...
      setChatLogs((prev) => [
        ...
        {
          sender: "ai",
          text: "Hệ thống gặp lỗi khi kết nối với mô hình đánh giá. Vui lòng thử gửi lại câu trả lời.",
        },
      ]);
    } finally {
      setLoading(false);
  ...
  const isFinished = currentIdx >= questions.length - 1 && questions[currentIdx]?.answer !== undefined;
      ...
      <div className="flex-1 flex flex-col bg-white border-4 border-black rounded-2xl neo-shadow-lg h-[600px] overflow-hidden">
        ...
        <div className="bg-[#FDF6E2] border-b-4 border-black p-4 flex justify-between items-center font-mono">
          ...
          <div className="flex items-center gap-2">
            <div className={`w-3 h-3 rounded-full border-2 border-black ${loading ? "bg-[#FFDE4D] animate-ping" : "bg-[#3DBC93]"}`}></div>
            <span className="text-xs font-black uppercase">
              {loading ? "AI IS THINKING..." : "AI STATUS: ACTIVE"}
            </span>
          </div>
        </div>
          ...
          {loading && (
            <div className="self-start flex gap-3 max-w-[80%]">
              ...
              <div className="border-4 border-black p-4 rounded-xl font-mono text-sm bg-white rounded-tl-none neo-shadow-sm flex items-center gap-2">
                <span className="font-black text-black">AI đang đánh giá câu trả lời của bạn...</span>
        ...
        <div className="border-t-4 border-black p-4 bg-white">
          {!isFinished ? (
            ...
            <button
              ...
              className="w-full py-4 bg-[#3DBC93] hover:bg-[#4ddbb0] text-black border-4 border-black font-black font-mono rounded-xl neo-shadow neo-btn uppercase tracking-wider flex items-center justify-center gap-2 text-base shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]"
            >
